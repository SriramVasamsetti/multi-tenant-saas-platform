...
exports.registerTenant = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ success: true, message: "Tenant registered successfully", data: { tenantId, subdomain, adminUser: userResult.rows[0] } });
  } catch (error) {
...
exports.login = async (req, res) => {
  ...
  const activeSubdomain = subdomain || tenantSubdomain; // Handle both naming conventions

  try {
    const result = await pool.query(
      `SELECT u.*, t.status as tenant_status FROM users u 
       LEFT JOIN tenants t ON u.tenant_id = t.id 
       WHERE u.email = $1 AND (t.subdomain = $2 OR u.tenant_id IS NULL)`,
